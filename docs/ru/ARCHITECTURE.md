...
Дополнительный [веб-сервер](DASHBOARD.md) отдаёт локальные HTML/CSS/JavaScript и читает тот же журнал через соединения SQLite только для чтения. Запросы браузера не инициализируют пространство, не меняют схему и не записывают сущности. Исходные поля сохраняются; вычисленная география дополняет отображение и не подменяет доказательства. Зарегистрированные документы доступны только внутри выбранного приватного пространства.
