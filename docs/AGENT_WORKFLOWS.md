...
| `preparation_overview` | `title`, `as_of`, `summary`, `basis`, `common[]`, `tracks[]` (both tracks: `positioning`, `market_notes`, `themes[]`, `interview_focus`, `target_vacancy_ids`), `evidence_to_verify[]`, a weekly `plan` (`hours_per_week`, `weeks[]` with `exercises[]`) and/or calendar-free `learning[]` areas (`id`, `family` = `common` or a `role_families[]` id, `priority` high/medium/low, `title`, `why`, `strength`, `learn[]`, `done_when`, `exercises[]`, optional `sources[]` https links) with `role_families[]` (`id`, `title`, `track`, `levels`, `summary`, `interview_focus`, `target_vacancy_ids`), `questions[]`, `stories[]`, `story_gaps`, `cv_advice[]`, `do_not`, `next_actions` | Strength labels follow fact verification; fact and vacancy IDs must exist; no coding exercises; changes no facts, CV or assessments |
| `relevance_review` | `reviews[]` with `vacancy_id`, `verdict` (`strong`, `possible`, `weak`, `off_profile`), `score` 0–100, `track`, `summary`, `reasons[]` (`kind`: `fit`, `gap`, `risk`; `text`), optional `fact_ids` | Flagship actor with a session; each score inside its verdict band; existing vacancies once each; the CLI stores digests of what was read and of the facts, so the review goes stale when either changes ([profile relevance](RELEVANCE.md#review-by-meaning)) |
