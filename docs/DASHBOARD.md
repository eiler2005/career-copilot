...
- **Layout.** A navy masthead holds the sections (with record counts) and the language, text and refresh controls; on a phone the section row scrolls sideways. Each page opens with a heading tile and a large section number, and content is laid out as a bento grid — tiles that share hairline borders — with mono labels, light display headings with an orange rule and orange primary actions. The palette is the Career Copilot navy, blue, orange and sage. Fonts are system stacks (Inter Tight, Inter and JetBrains Mono are used only if installed locally), so no font files are downloaded.
