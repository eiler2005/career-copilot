...
| Найти и оценить роли | `career-job-search` | Итоги сбора, вакансии с исходными условиями и объяснённым результатом соответствия |
...
Подробнее: [обзор системы](docs/ru/SYSTEM_OVERVIEW.md) — все модули, контейнеры и потоки данных на одной странице, [источники и официальные API](docs/ru/SOURCES.md), [данные](docs/ru/DATA_MODEL.md), [полный рабочий цикл](docs/ru/WORKFLOW.md).
...
Заявки интерфейса вступают в силу только после `ajh inbox apply` на машине, где хранится журнал. Практика только текстовая (без голоса и видео), модуль CRM пока только описан, новых адаптеров источников сверх шести перечисленных нет.
