...
The current visual fallback is dashboard design v3. It is used only when there is no valid saved preference; saved v1 and v2 choices remain selected. The footer **Design** selector offers v1, v2 and v3 and remembers the choice in `localStorage` under `career-copilot-design`. To choose via a link, open the dashboard with `?design=v1`, `?design=v2` or `?design=v3`; that choice is applied on load and persisted. An invalid value falls back to the saved valid choice, then to v3. Hash routes and record links are unchanged. If browser storage is unavailable, the selected design still applies for the current page.
...
- **Layout.** v3 uses a desktop left navigation rail, a light warm canvas, graphite type, mint surfaces and lime actions, with colorful stat tiles for a quick read of the workspace. The **Your next moves** shortcuts lead to roles, CV, preparation and pipeline; they are navigation choices rather than funnel completion or progress indicators. On a phone the rail becomes a horizontally scrollable navigation row. Cards and salary lines wrap on narrow screens, and mobile form fields use larger text. Fonts are system stacks, so no font files are downloaded.
- **Design versions.** v3 is the new overlay on top of the preserved v1 and v2 CSS layers. The visual direction was informed by [Web Dashboard Interface / finance](https://ru.pinterest.com/pin/17732992278406858/) and [Project Management Dashboard UI](https://ru.pinterest.com/pin/146085581660074230/); no artwork or assets are copied. Use the footer selector or the `design` query parameter to compare all three versions. Switching only changes browser presentation; it does not change the journal, requests, routes or data.
...
For a visual rollback only, choose v1 in the footer or open `?design=v1#vacancies`; the running application image and workspace stay in place, so no server redeploy is required. Rolling back the application image is independent: select the previous image and workspace snapshot, recreate the service and repeat the health/content checks when application behavior also needs to change. The Git release references `dashboard-design-v1` and `dashboard-design-v2` mark the visual baselines; the package version is separate. Do not delete volumes or overwrite retained document versions during an update. The dashboard does not alter the existing journal schema or require a reimport of legacy data.
