...
2. **Собрать.** Настройте `campaigns` ([настройки](CONFIGURATION.md#кампании-поиска)), запустите `discover` и прочитайте `collection runs`; отдельные вакансии добавляйте через `vacancy add --url` или `--text-file`. Возможные дубли проверяйте, а не сливайте автоматически.
