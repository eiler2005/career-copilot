...
The client ignores ambient proxy configuration. Proxy use requires `proxy_allowed: true` and `proxy_env` naming an environment variable for that source. `proxy_mode: "always"` (default) sends every request through the proxy; `proxy_mode: "fallback"` makes the proxy a reserve channel: the request goes directly first and is repeated once through the proxy only after a timeout or failed connection. An HTTP answer — 403, 429, an authentication wall or a challenge page — is never retried through the proxy. The route taken is recorded as `health.route` (`direct`, `proxy` or `proxy_fallback`); the proxy URL is never stored. The full decision flow is in [source architecture](SOURCE_ARCHITECTURE.md#network-routes-and-the-proxy). Keep credentials outside JSON and Git. Do not rotate proxies after blocks; no CAPTCHA bypass, login bypass or anti-bot evasion. LinkedIn proxy routes are forbidden. Browser and search fallback are agent/user work outside this CLI.
...
The step-by-step guide with the adapter contract and a code template is in [source architecture](SOURCE_ARCHITECTURE.md#add-a-new-source).
