...
9. **Фильтр** `include_title` — поисковый фильтр, а не оценка соответствия. `observed_total` считает карточки до фильтра, `count` — после.
...
13. **Запись состояния** (`source_health`: статус, причина сбоя, HTTP-статус, количество, маршрут, последний успех, следующая попытка) и в конце одна запись `collection_runs`: новые, изменившиеся, возможные дубли между источниками и ошибки.
