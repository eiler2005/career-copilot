...
[PDF-конвейер](PDF.md) разделяет содержание, вёрстку и хранение. `pdf_documents.py` предоставляет общий Markdown-рендерер, проверку, сборку и извлечение страниц PDF. В `workflow.py` остаются подготовка пакетов и правила ревью; `dashboard_pdf.py` адаптирует планы журнала и переводы для скачивания из браузера. CLI регистрирует постоянный результат внутри выбранного приватного пространства. Вёрстка не вызывает модель и не создаёт авторство или положительное ревью.

ReportLab выполняет вёрстку, pypdf читает и собирает страницы; Markdown разбирается локально. Общий слой обслуживает планы и CV, при этом правила их приёмки различаются. Автор отвечает за текст, рендерер — за разбиение на страницы и типографику, проверяющий осматривает фактические страницы. PDF — производный артефакт: его редактирование не обновляет план обучения и не доказывает прогресс. Прежние неизменяемые версии пакетов сохраняют исходные байты.
...
[cli.py](../../src/job_search_agent/cli.py), общий Markdown/PDF-рендерер —
[pdf_documents.py](../../src/job_search_agent/pdf_documents.py), подготовка и проверки пакета — [workflow.py](../../src/job_search_agent/workflow.py).
...
модели. Новые адаптеры подчиняются [SOURCES](SOURCES.md) и контракту из [архитектуры источников](SOURCE_ARCHITECTURE.md), изменения схемы и переносы
