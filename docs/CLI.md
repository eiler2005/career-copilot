...
| `relevance list [--relevant] [--tier T]` / `relevance explain ID` / `relevance search "QUERY"` / `relevance profile` / `relevance set PATH` | [Profile relevance](RELEVANCE.md): tiers with reasons, one vacancy explained, keyword queries such as `title:(engineer \| инженер) + (ai \| ии)`, the resolved rules, or replace `settings.json → relevance` |
