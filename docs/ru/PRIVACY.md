...
[Веб-интерфейс](DASHBOARD.md) предназначен для приватного просмотра. Docker-образ содержит публичный код и файлы интерфейса, а внешнее пространство подключается только для чтения при запуске. Сохраняйте привязку порта к loopback хоста и используйте SSH. Публичный домен использует только проверенный [HTTPS-шлюз с авторизацией](DASHBOARD.md#публичный-https-адрес-с-авторизацией); сам интерфейс напрямую не публикуется. Приватные HTML/JSON нельзя публиковать, пространство нельзя копировать в слой образа. Dockerfile, Compose и `.dockerignore` разрешены как публичная конфигурация и проходят те же проверки содержимого и секретов, что исходный код.
