...
**CLI** сохраняет входы и хеши, собирает поддержанные источники, применяет явные правила доказательств, строит документы, применяет заявки интерфейса и считает статистику. Общие [PDF-инструменты](docs/ru/PDF.md) превращают Markdown-планы в документы для чтения и печати, собирают PDF-сборники, проверяют файлы и извлекают страницы. В нём нет модельного API, автоматической отправки откликов или фонового планировщика.
...
| [PDF-документы](docs/ru/PDF.md) | Вёрстка планов, сборники, проверка и извлечение страниц, визуальный контроль |
...
PDF-рендерер поддерживает описанное подмножество Markdown; произвольный HTML/CSS и удалённые ресурсы не входят в его контракт. Происхождение модельного вклада записывается и проверяется структурно, но не удостоверяется провайдером модели. Конфигурация источников — доверенный приватный ввод; сетевые ограничения не являются полной изоляцией. Локальное хранение не шифрует backup автоматически и не разрешает передачу персональных данных hosted-сервисам.
