...
Регистрируйте каналы в приватном `sources`: `provider: telegram`, числовой `telegram-<id>`, публичное имя в `board`, `collection_mode: external_export`. `discover` возвращает `external_export_required` без HTTP и создания работодателя, сохраняя прошлые данные импорта. Отключённые источники остаются видимыми с причиной. Интерфейс разделяет время экспорта/импорта и показывает ограниченное покрытие; импорт не подтверждает доступность вакансии. См. [настройку](CONFIGURATION.md#зарегистрированные-telegram-каналы).
