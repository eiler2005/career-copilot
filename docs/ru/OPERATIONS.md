...
Для зафиксированной пакетной оценки храните манифест выборки рядом с приватным
журналом и включайте его в резервную копию. После восстановления сверьте ID и хеши
снимков источников с восстановленными данными, выполните `verify` и заново
сформируйте отчёт из восстановленного пространства перед продолжением. Копия
отчёта сама по себе не доказывает завершение оценки.
...
Типовые ситуации с новыми возможностями:

| Ситуация | Что проверить | Как исправить |
| --- | --- | --- |
| Заявка `conflict` | `inbox list --status conflict`, текущее состояние записи | Повторить действие от текущего состояния; старая заявка остаётся для аудита |
| Заявка `failed` | Поле `error` (например, нечитаемая страница у `vacancy_add`) | Исправить вход (вставить текст вместо заблокированной ссылки) и создать новую заявку |
| Задача зависла в `running` или `blocked` | `tasks list`, связанная `activity show ID` | Завершить активность как blocked/failed со следующим шагом или начать новую связанную активность |
| Оценка «требует обновления» | Изменившиеся части входов на вкладке «Соответствие» | `evaluate --stale` после проверки изменения |
| Кампании не применяются | `campaigns_error` в интерфейсе или `campaigns list` | Исправить список; некорректный список игнорируется целиком |

Миграции данных сентябрьских релизов 2026 года по умолчанию работают как пробный прогон и идемпотентны: `maintenance reextract-conditions [--apply]` извлекает условия вакансий из сохранённых текстов, `evaluate --stale` пересчитывает оценки с изменившимися входами. Сначала сделайте `backup`, после — `verify`.
