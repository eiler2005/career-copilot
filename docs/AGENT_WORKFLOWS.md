...
| `preparation_overview` | `title`, `as_of`, `summary`, `basis`, `common[]`, `tracks[]` (both tracks: `positioning`, `market_notes`, `themes[]`, `interview_focus`, `target_vacancy_ids`), `evidence_to_verify[]`, `plan` (`hours_per_week`, `weeks[]` with `exercises[]`), `questions[]`, `stories[]`, `story_gaps`, `cv_advice[]`, `do_not`, `next_actions` | Strength labels follow fact verification; fact and vacancy IDs must exist; no coding exercises; changes no facts, CV or assessments |
