...
Telegram-каналы входят в реестр источников с `collection_mode: external_export`; см. [регистрацию](CONFIGURATION.md#зарегистрированные-telegram-каналы). Для включённых каналов `discover` возвращает `external_export_required`, сохраняя время и покрытие прошлого импорта. Раздел источников показывает имя/ссылку, включение, причину отключения, даты экспорта/импорта и ограниченное покрытие.

[English](../SOURCES.md) · [Русский](SOURCES.md) · [Документация](../../README.ru.md#документация)

Пятнадцать адаптеров — ATS-доски работодателей, доски удалённой работы, региональные доски, открытые данные РФ и поиск HH по тексту — собирают опубликованные вакансии и поддерживают офлайн-разбор совместимых снимков. Официальные доски помогают проверять условия найма, карточки агрегатора добавляют находки и зарплатный контекст. Ни один маршрут не гарантирует полный охват рынка или актуальность ранее найденной вакансии.

Помимо этих пятнадцати HTTP-адаптеров, [сбор Telegram и импорт без сети](SOURCE_ARCHITECTURE.md#telegram-внешняя-сессия-ограниченный-экспорт-импорт-без-сети) используют ограниченный экспорт публичного канала через существующую внешнюю сессию. Команда `vacancy import-telegram PATH` сохраняет посты и происхождение данных в журнале; `discover` не подключается к Telegram. Импортированные посты — предварительные находки с неизвестной доступностью, а каналы не считаются работодателями.
...
| `hh` (поиск по тексту) | `query`; необязательно `area`, `period`, `professional_role`, `experience`, `schedule`; поиск вакансий, работодатель берётся из каждой карточки | [Документация HH API](https://api.hh.ru/openapi/redoc) |
| `smartrecruiters` | `board` (идентификатор компании); публичный список вакансий, пагинация `offset`; карточка без описания | [SmartRecruiters Posting API](https://developers.smartrecruiters.com/docs/posting-api) |
| `workable` | `board` (поддомен аккаунта); публичный виджет карьерной страницы; карточка без описания | [Виджет Workable](https://help.workable.com/hc/en-us/articles/115012771647) |
| `recruitee` | `board` (поддомен компании); публичный список вакансий с описанием, требованиями и зарплатой | [Recruitee Careers Site API](https://docs.recruitee.com/reference/intro-to-careers-site-api) |
| `remotive` | необязательно `category`, `query`, `limit`; удалённые вакансии, не чаще раза в шесть часов | [Remotive API](https://github.com/remotive-com/remote-jobs-api) |
| `remoteok` | необязательно `tag`; лента удалённых вакансий, первый элемент — юридическое уведомление провайдера | [Remote OK API](https://remoteok.com/api) |
| `jobicy` | необязательно `query` (тег), `geo`, `category`, `limit`; удалённые вакансии с географией, уровнем и годовой зарплатой | [Jobicy API](https://jobicy.com/jobs-rss-feed) |
| `arbeitnow` | без настроек; доска с данными из ATS работодателей (в основном Европа), ограниченная пагинация | [Arbeitnow API](https://www.arbeitnow.com/blog/job-board-api) |
| `getonbrd` | `query`; публичный поиск (в основном Латинская Америка), ограниченная пагинация | [Get on Board API](https://www.getonbrd.com/api-doc.html) |
| `trudvsem` | `query`; необязательно код `region`; открытые данные портала «Работа России», `limit=100`, контакты работодателя не копируются | [Открытые данные «Работа России»](https://trudvsem.ru/opendata/api) |

Доски и агрегаторы, добавленные 15 сентября 2026 года, находятся в [source_adapters.py](../../src/job_search_agent/source_adapters.py). Агрегаторы (`remotive`, `remoteok`, `jobicy`, `arbeitnow`, `getonbrd`, `trudvsem`, поиск HH по тексту) берут работодателя из каждой карточки; компания, которая уже есть в журнале с тем же названием или alias, используется повторно, поэтому один работодатель из нескольких источников остаётся одной компанией, а возможные дубли между источниками показываются в итоге сбора. Действуют условия провайдеров: ссылка на исходную вакансию и указание источника; Remotive просит несколько запросов в день, поэтому минимальный интервал — шесть часов. Страница проверки или HTTP 403 записывается как `blocked` и никогда не обходится. Из сети, использованной 15 сентября 2026 года, «Работа России», Workable и Recruitee ответили и проверены вживую; Jobicy, Remote OK и Get on Board периодически давали таймауты и были собраны через резервный канал (`proxy_mode: fallback`); из сети сервера ответили Remotive, Arbeitnow, Jobicy, Remote OK, Get on Board, Workable и Recruitee; Remotive, Arbeitnow и SmartRecruiters вернули проверку на бота, а HH отказал в запросах (и из сети сервера тоже), поэтому эти адаптеры следуют документированным форматам и требуют сети, которую провайдер принимает. Платные API агрегаторов (например, LoopCV) не подключены.
...
`proxy_mode: "always"` (по умолчанию) отправляет все запросы через прокси.
`proxy_mode: "fallback"` делает прокси резервным каналом: запрос сначала идёт
напрямую и повторяется один раз через прокси только после таймаута или сбоя
соединения. HTTP-ответ — 403, 429, требование авторизации или страница проверки —
через прокси не повторяется. Маршрут записывается в `health.route` (`direct`,
`proxy` или `proxy_fallback`); URL прокси не сохраняется. Полная схема решения —
в [архитектуре источников](SOURCE_ARCHITECTURE.md#сетевые-маршруты-и-прокси).
...
Пошаговая инструкция с контрактом адаптера и шаблоном кода — в [архитектуре источников](SOURCE_ARCHITECTURE.md#как-подключить-новый-источник).
