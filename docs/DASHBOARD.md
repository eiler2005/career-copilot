...
The current visual fallback is dashboard design v4. It is used only when there is no valid saved preference; saved v1, v2 and v3 choices remain selected. The footer **Design** selector offers v1, v2, v3 and v4 and remembers the choice in `localStorage` under `career-copilot-design`. To choose via a link, open the dashboard with `?design=v1`, `?design=v2`, `?design=v3` or `?design=v4`; that choice is applied on load and persisted. An invalid value falls back to the saved valid choice, then to v4. Hash routes and record links are unchanged. If browser storage is unavailable, the selected design still applies for the current page.
...
- **Layout.** v4 keeps the navigation rail and responsive behavior while introducing an executive business direction: light navigation, an almost white canvas, indigo accents, soft sans-serif headings, a compact metric strip and translucent surfaces. A lavender glow, dotted texture, subtle entry animations and hover elevation follow the Mercury Insights direction. Motion effects respect `prefers-reduced-motion`; there are no continuous animations or external visual assets. The **Your next moves** shortcuts lead to roles, CV, preparation and pipeline; they are navigation choices rather than funnel completion or progress indicators. On a phone the rail becomes a horizontally scrollable navigation row. Cards and salary lines wrap on narrow screens, and mobile form fields use larger text. Fonts are system stacks, so no font files are downloaded.
- **Design versions.** v4 layers the preserved v1, v2 and v3 CSS overlays and adds `styles-v4.css`. The business direction was informed by [Mercury Insights](https://mercury.com/insights), [Linear Features](https://linear.app/features) and [Pentagram — The Coterie](https://www.pentagram.com/work/the-coterie); no artwork or assets are copied. Use the footer selector or the `design` query parameter to compare all four versions. Switching only changes browser presentation; it does not change the journal, requests, routes or data.
- **Profile first.** Vacancies and the pipeline open on vacancies that fit the profile (**Relevant** = strong + possible); a switch shows each tier and **All** with counts, saved queries sit under the search box, and the search box accepts several words (all must appear) or queries with fields such as `company:(sber | сбер) + title:(director | head) + location:(moscow | remote) + text:llm`. Each card shows a 0–100 thermometer marked "by rules" or "by meaning" and why it fits or not; the **Fit** tab shows the agent's review, the four score parts and every reason, and **Review by meaning** queues an agent task. See [profile relevance](RELEVANCE.md).
...
- **Preparation** opens with a summary: active plans, topics attempted and reviewed, answers awaiting review and the next step (answer an existing question, or start practice for the first open week). The current **preparation overview** follows as a card with tabs — Summary, Product roles, Technical leadership, Unified plan, Questions, Stories, CV and evidence — with evidence labels on every theme, linked vacancies and facts, and a practice button on every exercise and question; its next open exercise becomes the next step. **My plans** shows compact cards — vacancy plans titled by the role and company, plans for general gaps and one shared interview-plan document per file — each with a progress bar; the full plan opens on demand. A vacancy plan lists weeks and gaps with status and a practice button (structural gaps link to clarification in the vacancy); questions and answers appear under the topic. The shared document has a table of contents, readable tables and authorship metadata kept secondary; PDF and Markdown downloads stay available. Below come vacancy briefs (company and interview claims labelled confirmed / participant report / assumption with dated sources, questions with provenance, STAR stories, plan and employer questions), free practice with answer drafts, highlighted review fragments, follow-up and retry, and **Start new preparation** (for a vacancy, grouped into vacancies with an assessment or plan and the rest, or for general gaps). A track switch replaces search on this page.
...
Containers, volumes, ports and hardening are summarised in the [system overview](SYSTEM_OVERVIEW.md#containers).
...
For a visual rollback only, choose v1, v2 or v3 in the footer, or open `?design=v3#vacancies`; the running application image and workspace stay in place, so no server redeploy is required. Rolling back the application image is independent: select the previous image and workspace snapshot, recreate the service and repeat the health/content checks when application behavior also needs to change. The Git release references `dashboard-design-v1`, `dashboard-design-v2` and `dashboard-design-v3` mark the preserved visual baselines; `dashboard-design-v4` marks this release, and the package version is separate. Do not delete volumes or overwrite retained document versions during an update. The dashboard does not alter the existing journal schema or require a reimport of legacy data.
