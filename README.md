...
The interface supports Russian and English, desktop and mobile, and needs no frontend build or external CDN. Dashboard design v2 is the default; the footer **Design** selector or `?design=v1` / `?design=v2` switches the visual layer immediately and persists the choice without changing hash routes or journal data. It never edits the journal directly: a decision, a vacancy link, a campaign or a practice answer is saved as a request, and authored or researched work becomes a task for an agent session that is marked done only after a finished activity. Run it locally or use the included Docker configuration with a private SSH tunnel or the authenticating gateway. Keep candidate data outside the image and public repository.
