...
Для `fits_verified` нужны `content_scope: full` или `page_text` и отсутствие явного `requirements_complete: false`. Для существующих полных описаний разметка по умолчанию считается полной; при незавершённой разметке задайте `requirements_complete: false`. Значение `true` не отменяет неполноту фрагмента, карточки или неизвестного описания. Подтверждённое обязательное несоответствие и конкретные открытые вопросы сохраняют приоритет; иначе неполное покрытие даёт `insufficient_data`.

Цепочка: **зарегистрированные источники → неизменяемые снимки → дедупликация → релевантность профилю → разметка требований → соответствие по фактам → предложения правок CV → независимая проверка**. Внешний экспорт/импорт Telegram — один из маршрутов сбора. Релевантность выбирает материалы для разбора, но не доказывает квалификацию. Неполные требования остаются неопределёнными даже при подходящем заголовке. До предложения правки свяжите требования с проверенными фактами кандидата. Принятые правки создают новую версию; готовность требует независимой проверки именно этой версии.

Если в объявлении указано несколько вариантов роли или альтернативные квалификации, сохраняйте смысл «ИЛИ» и необязательных требований; не превращайте все альтернативы в обязательную связку «И». До оценки выберите и нормализуйте фактическую роль либо зафиксируйте явную блокирующую необходимость уточнения, если движок не умеет представить альтернативы. Цитаты требований должны быть взяты из объявления целевого работодателя, а не из связанных вакансий, комментариев агрегатора или комментариев, созданных ИИ.

### Завершение зафиксированной выборки

Для повторяемой пакетной оценки до начала работы сохраните в приватном манифесте точные ID вакансий, ссылки и хеши снимков источников, охват оценки и использованные версии правил. Сделайте резервную копию, восстановите её в новый каталог и выполните `verify`, прежде чем считать выборку восстанавливаемой. Манифест и исходные материалы хранятся только во внешнем приватном пространстве.

Для каждого зафиксированного ID запишите результат или явную блокирующую причину, включая карточки, признанные нерелевантными профилю. Доступность источника, релевантность профилю, соответствие требованиям и готовность документа — отдельные решения: недоступность источника или нерелевантность не означают решения о соответствии. В цепочке activity сохраняйте фактические модель и сессию, хеши входов и результатов. После прерывания не переписывайте исходную activity; создайте связанную activity для оставшейся работы.

Считайте изменения CV предложениями и независимо проверяйте их до передачи или применения; оценка не применяет правки автоматически. Решение пользователя и `cv apply-edits` создают черновик, затем `prepare --based-on` создаёт версию. Независимо проверьте именно эту версию, прежде чем считать её готовой. Проверьте покрытие обоих мастер-треков, пересоздайте отчёт, выполните `verify`, проверьте целостность и границу приватных данных, а также выполните публичные проверки приватности для `worktree`, `index` и `history` перед передачей результата.
...
### От найденной вакансии до подготовки

1. **Синхронизировать заявки.** Если есть размещённый интерфейс, запустите `workspace/bin/publish-dashboard` или `inbox import` + `inbox apply`; разберите заявки `conflict` и `failed`. `tasks next` передаёт задачи в очереди подходящему навыку.
2. **Собрать.** Настройте `campaigns` ([настройки](CONFIGURATION.md#кампании-поиска)), запустите `discover` и прочитайте `collection runs`; отдельные вакансии добавляйте через `vacancy add --url` или `--text-file`. Возможные дубли проверяйте, а не сливайте автоматически. Затем оставьте подходящие профилю: `relevance list --relevant` (или вид по умолчанию в интерфейсе) показывает вакансии, в названии которых функция трека нужного уровня и домены кандидата; `relevance search "title:(engineer | инженер) + (ai | ии)"` находит роли по ключевым словам. Правила `settings.json → relevance` уточняйте по спискам слабых и неподходящих ([релевантность профилю](RELEVANCE.md)).
3. **Понять карточку.** Проверьте исходную вилку, формат, занятость, язык и где разрешено работать; неизвестное остаётся неизвестным.
4. **Решить, подходим ли.** Разметьте требования через `vacancy requirements ID PATH`, затем `evaluate --stale`. Идите по маршрутам: `cv_edit` → резюме, `preparation` → подготовка, `clarify` → спросить и записать ответ, `decision_basis` → принять решение. Личное «не интересно» фиксируется отдельно.
5. **Исправить или адаптировать резюме.** Предложения, решения и `cv apply-edits` дают черновик; `prepare --based-on` создаёт версию; содержательное и визуальное ревью делают её готовой ([CV-профили](CV_PROFILES.md#правка-master-и-адаптация-под-вакансию)).
6. **Подготовиться.** Памятка к конкретной вакансии или `prep plan` по общим пробелам, затем текстовая практика с разбором и повтором ([подготовка](PREPARATION.md#модуль-подготовка-два-входа-и-цикл-практики)).
