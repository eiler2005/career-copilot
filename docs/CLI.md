...
| `collection runs [--limit N]` | Latest collection summaries, including new vacancies per relevance tier |
| `relevance list [--relevant] [--tier T]` / `relevance explain ID` / `relevance search "QUERY"` / `relevance pending [--all]` / `relevance profile` / `relevance set PATH` | [Profile relevance](RELEVANCE.md): tiers with the 0–100 thermometer and reasons, one vacancy explained with its score parts and agent review, keyword queries with fields such as `company:(sber \| сбер) + title:(director \| head) + location:(moscow \| remote)`, vacancies waiting for a review by meaning, the resolved rules, or replace `settings.json → relevance` |
