...
Локальный обзор — `report/index.html` пространства. Исходники, PDF, текст, покрытие и задания автору находятся в `packages/`, результаты запуска — в `demo-results.json`. Frontend-сборка и сервер не нужны. [Начало работы](docs/ru/GETTING_STARTED.md) описывает приватную настройку и реальное авторство/ревью.
...
Read-only адаптеры поддерживают Greenhouse, Lever, Ashby, HH и корпоративные страницы со статическим JSON-LD. Исходные снимки позволяют повторить разбор без сети. Пустой результат, частичный охват, cooldown и ошибка учитываются отдельно: заблокированная страница не превращается в «вакансий нет».
