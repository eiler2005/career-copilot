...
(() => {
  ...
  const enums = {
    external_export_required: ["Нужен внешний экспорт", "External export required"], imported: ["Импортирован", "Imported"],
  ...
  function renderResults() {
    ...
    // Several plain words must all appear (in any order); an operator or a field turns the search into a query.
    const words = query.split(/\s+/).filter(Boolean), haystack = (record) => `${recordTitle(record)} ${companyName(record)} ${record.kind === "vacancies" ? Object.values(recordFields(record)).join(" ") : ""} ${JSON.stringify(record.payload)}`.toLocaleLowerCase(state.lang);
    const matchesSearch = (record) => !query || (keywordQuery ? matchKeywordQuery(keywordQuery, recordFields(record)).matched : ((text) => words.every((word) => text.includes(word)))(haystack(record)));
  ...
  Object.assign(extraCopy.ru, {basis_incomplete_requirements: "Описание или разметка требований неполны"});
  Object.assign(extraCopy.en, {basis_incomplete_requirements: "The description or requirement annotation is incomplete"});
  function telegramCoverage(health) {
    const c = health.coverage || {}, tr = (ru, en) => state.lang === "ru" ? ru : en;
    const count = (value) => Number.isInteger(value) && value >= 0;
    const parts = [c.complete === true ? tr("Период просмотрен полностью", "Requested window fully scanned") : c.complete === false ? tr("Ограниченная выборка", "Bounded sample") : tr("Полнота неизвестна", "Completeness unknown")];
    if (count(c.scanned)) parts.push(`${tr("Просмотрено", "Scanned")}: ${c.scanned}`);
    if (count(health.observed_total)) parts.push(`${tr("Получено сообщений", "Messages received")}: ${health.observed_total}`);
    if (count(c.limit)) parts.push(`${tr("Лимит", "Limit")}: ${c.limit}`);
    if (c.since && !Number.isNaN(new Date(c.since).valueOf())) parts.push(`${tr("С", "Since")} ${formatDateTime(c.since)}`);
    return parts.join(" · ");
  }
  function sourceCard(record) {
    ...
    if (p.provider === "telegram") {
      titleArea.append(el("p", "company-label", "Telegram"), button(p.name || health.name || p.board || recordTitle(record), "record-title", () => openRecord(record)));
      top.append(titleArea, badge(status(record))); card.append(top);
      const tr = (ru, en) => state.lang === "ru" ? ru : en;
      card.append(factList([
        [tr("Сбор", "Collection"), p.enabled === false ? tr("Отключён", "Disabled") : tr("Внешний экспорт", "External export")],
        [tr("Причина отключения", "Disabled reason"), p.disabled_reason || ""],
        [tr("Экспорт", "Exported"), health.exported_at ? formatDateTime(health.exported_at) : t("never")],
        [tr("Импорт", "Imported"), health.imported_at ? formatDateTime(health.imported_at) : t("never")],
        [tr("Покрытие", "Coverage"), telegramCoverage(health)],
        [t("foundCount"), health.count ?? ""],
        [t("sourceProblem"), health.config_error || (health.status === "external_export_required" ? tr("Нужен внешний экспорт и импорт", "External export and import required") : "")],
      ]));
      const url = p.source_verified_url || ((p.board || health.board) ? `https://t.me/${encodeURIComponent(p.board || health.board)}` : "");
      const link = externalLink(url, `${p.board || health.board || "Telegram"} ↗`);
    ...
    card.append(factList([[label("market"), p.market ? translated(p.market) : ""], [t("schedule"), p.enabled === false ? translated("disabled") : scheduleText(p.interval_seconds)], [t("lastCheck"), health.last_attempt ? formatDateTime(health.last_attempt) : t("never")], [t("nextCheck"), health.next_attempt && p.enabled !== false ? formatDateTime(health.next_attempt) : ""], [t("foundCount"), health.count ?? ""], [t("titleFilter"), scalar(p.include_title)], [t("sourceProblem"), [health.failure_status || (health.status && !String(health.status).startsWith("success") && !["never_checked", "cooldown"].includes(health.status) ? translated(health.status) : ""), health.http_status && !String(health.status).startsWith("success") ? `HTTP ${health.http_status}` : "", scalar(health.config_error)].filter(Boolean).join(" · ")], [t("lastSuccess"), health.last_attempt ? (health.last_success ? formatDateTime(health.last_success) : t("never")) : ""], [t("sourceRoute"), health.route ? t(`route_${health.route}`) : ""]]));
  ...
  Object.assign(copy.ru, {
    ...
    sourceProblem: "Проблема", campaignsInvalid: "Кампании в settings.json не применяются из-за ошибки", lastSuccess: "Последний успех", sourceRoute: "Маршрут", route_direct: "напрямую", route_proxy: "через прокси", route_proxy_fallback: "резервный канал (прокси после сбоя соединения)", marketAll: "Все", marketRu: "РФ", marketIntl: "Международные", marketUnknown: "Рынок не определён"
  });
  Object.assign(copy.en, {
    ...
    sourceProblem: "Problem", campaignsInvalid: "Campaigns in settings.json are ignored because of an error", lastSuccess: "Last success", sourceRoute: "Route", route_direct: "direct", route_proxy: "through proxy", route_proxy_fallback: "reserve channel (proxy after a failed connection)", marketAll: "All", marketRu: "Russia", marketIntl: "International", marketUnknown: "Market unknown"
  });
  Object.assign(enums, {
    ...
    ru: ["РФ", "Russia"], intl: ["Международный", "International"], any: ["Любой", "Any"], not_interested: ["Не интересно", "Not interested"], interested: ["Интересно", "Interested"], greenhouse: ["Greenhouse", "Greenhouse"], intake: ["Добавлена вручную", "Added manually"], lever: ["Lever", "Lever"], ashby: ["Ashby", "Ashby"], hh: ["HH.ru", "HH.ru"], corporate: ["Сайт компании", "Company site"], linkedinsalaries: ["LinkedIn Salaries", "LinkedIn Salaries"], smartrecruiters: ["SmartRecruiters", "SmartRecruiters"], workable: ["Workable", "Workable"], recruitee: ["Recruitee", "Recruitee"], remotive: ["Remotive", "Remotive"], remoteok: ["Remote OK", "Remote OK"], jobicy: ["Jobicy", "Jobicy"], arbeitnow: ["Arbeitnow", "Arbeitnow"], getonbrd: ["Get on Board", "Get on Board"], trudvsem: ["Работа России", "Work in Russia (trudvsem)"]
  });
  ...
  // Field prefixes and their Russian aliases, as in `ajh relevance search`.
  const QUERY_FIELDS = {title: "title", role: "title", "должность": "title", "название": "title", company: "company", "компания": "company", location: "location", "локация": "location", "где": "location", text: "text", tech: "text", "текст": "text", "стек": "text"};
  const QUERY_FIELD_RE = new RegExp(`(^|[\\s+(|-])(${Object.keys(QUERY_FIELDS).join("|")})\\s*:`, "iu");
  // Same grammar as `ajh relevance search`: + joins groups, | alternatives, -word excludes, field: limits a group.
  function parseKeywordQuery(raw) {
    const query = String(raw || "").trim();
    if (!query || query.length > 300 || !(/[+|]|(^|\s)-\S/.test(query) || QUERY_FIELD_RE.test(query))) return null;
    const parts = query.split(/(?:^|\s)-(?=\s*\S)/);
    const group = (chunk) => {
      let text = chunk, field = "any"; const prefix = /^\s*([\p{L}]+)\s*:/u.exec(chunk);
      if (prefix && QUERY_FIELDS[prefix[1].toLowerCase()]) { field = QUERY_FIELDS[prefix[1].toLowerCase()]; text = chunk.slice(prefix[0].length); }
      const terms = text.replace(/[()]/g, " ").split(/[|,]/).map(normText).filter(Boolean); return terms.length ? {field, terms} : null;
    };
  ...
  function matchKeywordQuery(parsed, fields) {
    const everything = ["title", "company", "location", "text"].map((name) => fields[name] || "").join(" "), where = (item) => item.field === "any" ? everything : fields[item.field] || "", terms = [];
  ...
  function recordFields(record) {
    const geo = geography(record), allowed = record.payload.conditions?.allowed_geography?.countries || [], mode = conditionValue(record, "work_mode");
    return {
      title: normText(`${record.payload.title || ""} ${recordTitle(record)}`),
      company: normText(`${companyName(record)} ${record.payload.company_name || ""}`),
      location: normText([geo.raw, geo.country, geo.city, geo.remote, geo.country !== "unknown" ? countryName(geo.country) : "", mode, ...allowed].filter((value) => value && value !== "unknown").join(" ")),
  ...
  function reasonText(reason) {
    ...
    if (reason.code === "cv_support") return `${base}: ${(reason.domains || []).map((item) => `${domainLabel(item.id)} — ${item.facts} ${plural(item.facts, ["факт", "факта", "фактов", "fact", "facts"])}`).join(", ")}`;
    if (reason.code === "query_included") return `${base} «${reason.query}»`;
    return words ? `${base} (${words})` : base;
  }
  function relevanceSummary(result) {
    if (result.method === "agent" && result.review?.summary) return tx(result.review.summary);
  ...
  // Thermometer: 0–100 with the tier bands (weak < 50 ≤ possible < 70 ≤ strong); the method says who scored it.
  function thermometer(result, large = false) {
    const box = el("span", `thermo tier-${result.tier}${large ? " large" : ""}`), bar = el("span", "thermo-bar"), fill = el("span", "thermo-fill");
    fill.style.width = `${Math.max(2, Math.min(100, result.score))}%`; bar.append(fill); bar.setAttribute("role", "img"); bar.setAttribute("aria-label", `${t("relevanceScore")} ${result.score}/100`);
    box.append(bar, el("span", "thermo-value", `${result.score}`), el("span", "thermo-method", result.method === "agent" ? t("byMeaning") : t("byRules")));
    return box;
  }
  function relevanceLine(record) {
    ...
    line.append(el("span", "vc-fit-label", `${t("relevanceLabel")}:`), badge(result.tier), thermometer(result), el("span", "vc-relevance-why", relevanceSummary(result)));
    if (result.review?.stale) line.append(el("span", "badge attention", t("reviewStale")));
    return line;
  }
  function relevancePanel(record) {
    ...
    const head = el("div", "fit-head"); head.append(badge(result.tier), thermometer(result, true)); section.append(head);
    const review = result.review;
    if (review) {
      const box = el("div", `semantic-review${review.stale ? " is-stale" : ""}`);
      box.append(el("div", "eyebrow", `${t("semanticReview")} · ${review.model || ""}${review.created_at ? ` · ${formatDateTime(review.created_at)}` : ""}`), el("p", "", tx(review.summary || "")));
      const list = el("ul", "criteria-list"); (review.reasons || []).forEach((item) => { const row = el("li"); row.append(el("span", `badge ${item.kind === "fit" ? "good" : item.kind === "risk" ? "attention" : ""}`, t(`reviewKind_${item.kind}`)), el("span", "", tx(item.text))); list.append(row); }); box.append(list);
      if (review.stale) box.append(el("p", "detail-warning", t("reviewStaleHelp")));
      else if (result.rules) box.append(el("p", "muted small-note", `${t("rulesSaid")}: ${translated(result.rules.tier)} · ${result.rules.score}/100`));
      section.append(box);
    }
    const parts = result.parts || {}, partsBox = el("div", "thermo-parts");
    [["role", 30], ["level", 25], ["domains", 25], ["evidence", 20]].forEach(([key, max]) => { const row = el("div", "thermo-part"), bar = el("span", "thermo-bar"), fill = el("span", "thermo-fill"); fill.style.width = `${Math.round(100 * (parts[key] || 0) / max)}%`; bar.append(fill); row.append(el("span", "thermo-part-name", t(`part_${key}`)), bar, el("span", "mono", `${parts[key] ?? 0}/${max}`)); partsBox.append(row); });
    section.append(el("h4", "", review && !review.stale ? t("rulesBreakdown") : t("scoreBreakdown")), partsBox);
    ...
    section.append(requestButton(review && !review.stale ? t("requestSemanticAgain") : t("requestSemantic"), "quiet-button", () => ({type: "task", payload: {task_type: "review_relevance", related: {vacancy_id: record.id}}}), t("taskQueued")));
  ...
  Object.assign(copy.ru, {
    ...
    weekResult: "Результат недели", overviewEyebrow: "Общий обзор подготовки", bothTracks: "Оба направления", basisVacancies: "вакансий в основе", overviewTab_summary: "Сводка", "overviewTab_product": "Продуктовые роли", "overviewTab_technical-leadership": "Техническое лидерство", overviewTab_plan: "Единый план", overviewTab_learning: "Обучение", learningCommon: "Общее обучение", learningNote: "Темы без календаря: сначала высокий приоритет. Тема закрыта, когда выполнено условие «Готово, когда» и упражнение разобрано — не по времени чтения.", learnWhat: "Что прокачать", doneWhen: "Готово, когда", familyLevels: "Уровни", learningAreasCount: "тем обучения", roleFamiliesCount: "семейств ролей", priority_high: "высокий приоритет", priority_medium: "средний приоритет", priority_low: "низкий приоритет", overviewTab_questions: "Вопросы", overviewTab_stories: "Истории", overviewTab_cv: "Резюме и доказательства", overviewNextActions: "Что сделать в первую очередь", overviewCommon: "Общие выводы для обоих направлений", overviewMarket: "Рынок и спрос", overviewThemes: "Что требуют вакансии и что у вас есть", overviewInterviewFocus: "На чём будут проверять", overviewTargets: "Приоритетные открытые вакансии", overviewGoals: "Цели", overviewTasks: "Задачи вне практики", overviewExercises: "Упражнения", overviewQuestionsNote: "Банк вопросов для текстовой практики. Ответ уходит на разбор агенту; практика не создаёт опыт.", overviewUseFor: "Для каких вопросов", overviewCaution: "Осторожно", overviewStoriesNote: "Истории строятся только на записанных фактах; метрики клиентских программ не выдаются за личный результат.", overviewVerify: "Подтвердить доказательствами", overviewHow: "Как подтвердить", overviewCvAdvice: "Советы по резюме", overviewDoNot: "Чего не делать", overviewNote: "Обзор составлен агентом по фактам и вакансиям журнала. Метка силы следует статусу фактов: «подтверждено» — только проверенные факты. Обзор не меняет факты, резюме и оценки.", inVacancies: "В вакансиях", strength_verified: "подтверждено", strength_reported: "со слов кандидата", strength_gap: "пробел", strength_unknown: "неизвестно", overviewAction_verify_evidence: "подтвердить", overviewAction_cv_edit: "резюме", overviewAction_preparation: "подготовка", overviewAction_clarify: "уточнить",
  ...
  Object.assign(copy.en, {
    ...
    weekResult: "Week result", overviewEyebrow: "Preparation overview", bothTracks: "Both tracks", basisVacancies: "vacancies analysed", overviewTab_summary: "Summary", "overviewTab_product": "Product roles", "overviewTab_technical-leadership": "Technical leadership", overviewTab_plan: "Unified plan", overviewTab_learning: "Learning", learningCommon: "Common learning", learningNote: "Areas without a calendar: high priority first. An area is done when its \"done when\" condition is met and an exercise has been reviewed — not by reading time.", learnWhat: "What to build up", doneWhen: "Done when", familyLevels: "Levels", learningAreasCount: "learning areas", roleFamiliesCount: "role families", priority_high: "high priority", priority_medium: "medium priority", priority_low: "low priority", overviewTab_questions: "Questions", overviewTab_stories: "Stories", overviewTab_cv: "CV and evidence", overviewNextActions: "Do first", overviewCommon: "Shared conclusions for both tracks", overviewMarket: "Market and demand", overviewThemes: "What vacancies require and what you have", overviewInterviewFocus: "What interviews will probe", overviewTargets: "Priority open vacancies", overviewGoals: "Goals", overviewTasks: "Tasks outside practice", overviewExercises: "Exercises", overviewQuestionsNote: "Question bank for text practice. Answers go to an agent for review; practice creates no experience.", overviewUseFor: "Use for", overviewCaution: "Caution", overviewStoriesNote: "Stories use recorded facts only; client-programme metrics are not presented as personal results.", overviewVerify: "Back with evidence", overviewHow: "How to verify", overviewCvAdvice: "CV advice", overviewDoNot: "Do not", overviewNote: "Agent-authored from the journal's facts and vacancies. Strength labels follow fact verification: 'verified' uses verified facts only. It changes no facts, CV or assessments.", inVacancies: "In vacancies", strength_verified: "verified", strength_reported: "candidate-reported", strength_gap: "gap", strength_unknown: "unknown", overviewAction_verify_evidence: "verify", overviewAction_cv_edit: "CV", overviewAction_preparation: "preparation", overviewAction_clarify: "clarify",
  ...
  function prepSummary(list) {
    ...
    const overviewRecord = currentOverview();
    if (overviewRecord) { const ids = [...overviewExercises(overviewRecord.payload).map((item) => item.exercise.id), ...(overviewRecord.payload.questions || []).map((item) => item.id)]; const progress = progressOf(ids, planStatuses(overviewRecord)); total += progress.total; attempted += progress.attempted; reviewed += progress.reviewed; }
    ...
    } else if (overviewRecord && (() => { const statuses = planStatuses(overviewRecord); for (const item of overviewExercises(overviewRecord.payload)) if (topicState("preparation_overviews", overviewRecord.id, item.exercise.id, statuses) === "open") { next = {overview: overviewRecord, ...item}; return true; } return false; })()) {
      nextBox.append(el("p", "prep-next-title", next.week ? `${t("weekShort")} ${next.week.week}: ${next.exercise.text}` : `${next.area.title}: ${next.exercise.text}`), el("div", "stat-note", `${t("tests")}: ${next.exercise.tests}`), practiceButton({track: next.exercise.track === "both" ? undefined : next.exercise.track, plan_id: next.overview.id, plan_kind: "preparation_overviews", topic_id: next.exercise.id, question: next.exercise.text, type: next.exercise.type, tests: next.exercise.tests}, t("startPractice"), "primary-button"));
  ...
  // Unified preparation overview across both tracks (agent-authored, evidence-labelled).
  let overviewTab = "summary";
  const OVERVIEW_TABS = ["summary", "product", "technical-leadership", "learning", "plan", "questions", "stories", "cv"];
  const PRIORITY_ORDER = {high: 0, medium: 1, low: 2};
  // Practice topics of an overview in reading order: weekly plan, learning areas by priority, questions.
  const learningAreas = (p) => [...(p.learning || [])].sort((a, b) => (PRIORITY_ORDER[a.priority] ?? 1) - (PRIORITY_ORDER[b.priority] ?? 1));
  const overviewExercises = (p) => [...(p.plan?.weeks || []).flatMap((week) => (week.exercises || []).map((exercise) => ({exercise, week}))), ...learningAreas(p).flatMap((area) => (area.exercises || []).map((exercise) => ({exercise, area})))];
  const overviewTabs = (p) => OVERVIEW_TABS.filter((name) => (name !== "learning" || (p.learning || []).length) && (name !== "plan" || (p.plan?.weeks || []).length));
  const currentOverview = () => prepRecords("preparation_overviews").sort((a, b) => scalar(b.payload.created_at).localeCompare(scalar(a.payload.created_at)))[0];
  function strengthBadge(strength) { return el("span", `badge ${strength === "verified" ? "good" : strength === "reported" ? "blue" : strength === "gap" ? "attention" : ""}`, t(`strength_${strength}`)); }
  function factChips(ids) { if (!ids?.length) return ""; const box = el("span", "fact-chips"); ids.forEach((id) => box.append(el("code", "", id))); return box; }
  function vacancyChips(ids) { const found = (ids || []).map((id) => byId(id, "vacancies")).filter(Boolean); if (!found.length) return ""; const box = el("div", "stacked-refs"); found.forEach((vacancy) => box.append(vacancyReference(vacancy, {compact: true, date: false}))); return box; }
  function themeList(items) {
    const list = el("ol", "fit-matrix");
    (items || []).forEach((item) => {
      const row = el("li", `fit-row is-${item.strength === "verified" ? "match" : item.strength === "gap" ? "gap" : "unknown"}`), head = el("div", "fit-row-head");
      head.append(el("strong", "", item.title)); const tags = el("span", "card-meta"); tags.append(strengthBadge(item.strength)); if (item.action?.type && item.action.type !== "none") tags.append(el("span", "badge blue", t(`overviewAction_${item.action.type}`))); head.append(tags); row.append(head);
      row.append(factList([[t("topicWhy"), item.why || ""], [t("nextStep"), item.action?.text || ""], [t("editFacts"), factChips(item.fact_ids)], [t("inVacancies"), vacancyChips(item.vacancy_ids)]]));
      list.append(row);
    });
    return list;
  }
  function exerciseRow(record, exercise, statuses) {
    const row = el("li", "exercise-row"), state = topicState("preparation_overviews", record.id, exercise.id, statuses), head = el("div", "fit-row-head");
    head.append(el("strong", "", exercise.text)); const tags = el("span", "card-meta"); tags.append(el("span", "badge", t(`qtype_${exercise.type}`)), el("span", "badge blue", exercise.track === "both" ? t("bothTracks") : translated(exercise.track)), el("span", `badge${state === "reviewed" ? " good" : state === "open" ? "" : " blue"}`, t(`topic_${state}`))); head.append(tags); row.append(head);
    row.append(el("p", "muted small-note", `${t("tests")}: ${exercise.tests}`));
    const bar = el("div", "inline-form"); bar.append(practiceButton({track: exercise.track === "both" ? undefined : exercise.track, plan_id: record.id, plan_kind: "preparation_overviews", topic_id: exercise.id, question: exercise.text, type: exercise.type, tests: exercise.tests}, state === "open" ? t("practiceTopic") : t("anotherQuestion"))); row.append(bar);
    const nested = nestedSessions("preparation_overviews", record.id, exercise.id); if (nested) row.append(nested);
    return row;
  }
  function overviewView(record) {
    const p = record.payload, statuses = planStatuses(record), card = el("article", "plan overview-card"); card.dataset.recordId = record.id;
    const ids = [...overviewExercises(p).map((item) => item.exercise.id), ...(p.questions || []).map((item) => item.id)];
    const header = el("header", "plan-header"), heading = el("div", "plan-heading");
    const scope = (p.plan?.weeks || []).length ? `${p.plan.hours_per_week} ${t("hoursShort")}/${t("weeksShort")} × ${p.plan.weeks.length} ${t("weeksShort")}` : `${t("learningAreasCount")}: ${(p.learning || []).length} · ${t("roleFamiliesCount")}: ${(p.role_families || []).length}`;
    heading.append(el("p", "eyebrow", `${t("overviewEyebrow")} · ${p.as_of}`), el("h2", "plan-title", p.title), el("p", "plan-subtitle", `${t("bothTracks")} · ${scope} · ${t("basisVacancies")}: ${(p.basis?.vacancy_ids || []).length}`));
    header.append(heading); card.append(header, progressBar(progressOf(ids, statuses)));
    const tabs = el("div", "tabs"), panel = el("div", "tab-panel overview-panel"); tabs.setAttribute("role", "tablist"); tabs.setAttribute("aria-label", t("overviewEyebrow"));
    const shownTabs = overviewTabs(p);
    const draw = () => {
      if (!shownTabs.includes(overviewTab)) overviewTab = "summary";
      tabs.replaceChildren(...shownTabs.map((name, index) => { const tab = button(t(`overviewTab_${name}`), `tab${name === overviewTab ? " active" : ""}`, () => { overviewTab = name; draw(); }); tab.setAttribute("role", "tab"); tab.setAttribute("aria-selected", String(name === overviewTab)); tab.tabIndex = name === overviewTab ? 0 : -1; tab.addEventListener("keydown", (event) => { const target = event.key === "ArrowRight" ? index + 1 : event.key === "ArrowLeft" ? index - 1 : null; if (target === null) return; event.preventDefault(); overviewTab = shownTabs[(target + shownTabs.length) % shownTabs.length]; draw(); tabs.querySelector(".tab.active")?.focus(); }); return tab; }));
      const body = document.createDocumentFragment();
      if (overviewTab === "summary") {
        body.append(renderMarkdown(p.summary));
        if ((p.next_actions || []).length) { const list = el("ol", "next-actions"); p.next_actions.forEach((item) => list.append(el("li", "", item))); body.append(el("h3", "plan-section-title", t("overviewNextActions")), list); }
        if ((p.common || []).length) body.append(el("h3", "plan-section-title", t("overviewCommon")), themeList(p.common));
        if (p.basis?.notes) body.append(el("p", "plan-note", p.basis.notes));
      } else if (TRACK_OPTIONS.includes(overviewTab)) {
        const section = (p.tracks || []).find((item) => item.track === overviewTab) || {};
        body.append(el("p", "fit-reason", section.positioning || ""));
        if ((section.market_notes || []).length) { const list = el("ul", "plain-list"); section.market_notes.forEach((item) => list.append(el("li", "", item))); body.append(el("h3", "plan-section-title", t("overviewMarket")), list); }
        body.append(el("h3", "plan-section-title", t("overviewThemes")), themeList(section.themes));
        if ((section.interview_focus || []).length) { const list = el("ul", "plain-list"); section.interview_focus.forEach((item) => list.append(el("li", "", item))); body.append(el("h3", "plan-section-title", t("overviewInterviewFocus")), list); }
        const targets = vacancyChips(section.target_vacancy_ids); if (targets) body.append(el("h3", "plan-section-title", t("overviewTargets")), targets);
      } else if (overviewTab === "learning") {
        const areas = learningAreas(p), families = p.role_families || [];
        const areaCard = (area) => {
          const row = el("li", `fit-row is-${area.strength === "verified" ? "match" : area.strength === "gap" ? "gap" : "unknown"}`), head = el("div", "fit-row-head");
          head.append(el("strong", "", area.title)); const tags = el("span", "card-meta"); tags.append(el("span", `badge ${area.priority === "high" ? "attention" : area.priority === "low" ? "" : "blue"}`, t(`priority_${area.priority}`)), strengthBadge(area.strength)); head.append(tags); row.append(head);
          const learn = el("ul", "plain-list"); (area.learn || []).forEach((item) => learn.append(el("li", "", item)));
          const links = (area.sources || []).length ? el("ul", "plain-list") : ""; (area.sources || []).forEach((source) => { const li = el("li"), a = el("a", "", source.title); a.href = source.url; a.target = "_blank"; a.rel = "noopener noreferrer"; li.append(a); links.append(li); });
          row.append(factList([[t("topicWhy"), area.why || ""], [t("learnWhat"), learn], [t("doneWhen"), area.done_when || ""], [t("topicMaterials"), links], [t("editFacts"), factChips(area.fact_ids)], [t("inVacancies"), vacancyChips(area.vacancy_ids)]]));
          if ((area.exercises || []).length) { const list = el("ol", "exercise-list"); area.exercises.forEach((item) => list.append(exerciseRow(record, item, statuses))); row.append(el("h4", "", t("overviewExercises")), list); }
          return row;
        };
        const section = (title, items, intro) => { const box = el("section", "overview-week"); box.append(el("h3", "plan-section-title", title)); if (intro) box.append(intro); if (items.length) { const list = el("ol", "fit-matrix"); items.forEach((area) => list.append(areaCard(area))); box.append(list); } return box; };
        body.append(el("p", "muted", t("learningNote")), section(t("learningCommon"), areas.filter((area) => area.family === "common")));
        families.forEach((family) => {
          const intro = document.createDocumentFragment(); const tags = el("div", "card-meta"); tags.append(el("span", "badge blue", family.track === "both" ? t("bothTracks") : translated(family.track))); if (family.levels) tags.append(el("span", "muted small-note", `${t("familyLevels")}: ${family.levels}`)); intro.append(tags, el("p", "fit-reason", family.summary || ""));
          if ((family.interview_focus || []).length) { const list = el("ul", "plain-list"); family.interview_focus.forEach((item) => list.append(el("li", "", item))); intro.append(el("h4", "", t("overviewInterviewFocus")), list); }
          const targets = vacancyChips(family.target_vacancy_ids); if (targets) intro.append(el("h4", "", t("overviewTargets")), targets);
          body.append(section(family.title, areas.filter((area) => area.family === family.id), intro));
        });
      } else if (overviewTab === "plan") {
        (p.plan?.weeks || []).forEach((week) => {
          const box = el("section", "overview-week"), head = el("div", "fit-head"); head.append(el("h3", "", `${t("weekShort")} ${week.week}: ${week.theme}`), el("span", "badge blue", week.track === "both" ? t("bothTracks") : translated(week.track))); box.append(head);
          if ((week.goals || []).length) { const list = el("ul", "plain-list"); week.goals.forEach((item) => list.append(el("li", "", item))); box.append(el("h4", "", t("overviewGoals")), list); }
          if ((week.tasks || []).length) { const list = el("ul", "plain-list"); week.tasks.forEach((item) => list.append(el("li", "", item))); box.append(el("h4", "", t("overviewTasks")), list); }
          if ((week.exercises || []).length) { const list = el("ol", "exercise-list"); week.exercises.forEach((item) => list.append(exerciseRow(record, item, statuses))); box.append(el("h4", "", t("overviewExercises")), list); }
          box.append(el("p", "muted small-note", `${t("weekResult")}: ${week.deliverable}`)); body.append(box);
        });
      } else if (overviewTab === "questions") {
        const list = el("ol", "exercise-list"); (p.questions || []).forEach((item) => list.append(exerciseRow(record, item, statuses))); body.append(el("p", "muted", t("overviewQuestionsNote")), list);
      } else if (overviewTab === "stories") {
        const list = el("ol", "fit-matrix");
        (p.stories || []).forEach((story) => { const row = el("li", "fit-row is-unknown"), head = el("div", "fit-row-head"); head.append(el("strong", "", story.title)); const tags = el("span", "card-meta"); story.tracks.forEach((track) => tags.append(badge(track))); head.append(tags); row.append(head, factList([[t("overviewUseFor"), story.use_for], [t("overviewCaution"), story.caution || ""], [t("editFacts"), factChips(story.fact_ids)]])); list.append(row); });
        body.append(el("p", "muted", t("overviewStoriesNote")), list);
        if ((p.story_gaps || []).length) { const gaps = el("ul", "plain-list"); p.story_gaps.forEach((item) => gaps.append(el("li", "", item))); body.append(el("h3", "plan-section-title", t("storyGaps")), gaps); }
      } else {
        if ((p.evidence_to_verify || []).length) { const rows = [[t("editFacts"), t("topicWhy"), t("overviewHow")], ...p.evidence_to_verify.map((item) => [el("code", "", item.fact_id), item.why, item.how])]; body.append(el("h3", "plan-section-title", t("overviewVerify")), tableFrom(rows)); }
        if ((p.cv_advice || []).length) { const list = el("ul", "criteria-list"); p.cv_advice.forEach((item) => { const li = el("li"); li.append(el("span", "badge blue", item.track === "both" ? t("bothTracks") : translated(item.track)), el("span", "", item.text)); const chips = factChips(item.fact_ids); if (chips) li.append(chips); list.append(li); }); body.append(el("h3", "plan-section-title", t("overviewCvAdvice")), list); }
        if ((p.do_not || []).length) { const list = el("ul", "do-not-list"); p.do_not.forEach((item) => list.append(el("li", "", item))); body.append(el("h3", "plan-section-title", t("overviewDoNot")), list); }
      }
      panel.replaceChildren(body);
    };
    draw();
    card.append(tabs, panel, el("p", "muted small-note", t("overviewNote")));
    return card;
  }
  ...
  function renderPreparationModule(list) {
    ...
    const overview = currentOverview(); if (overview) container.append(overviewView(overview));
