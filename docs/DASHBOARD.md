...
- **Profile first.** Vacancies and the pipeline open on vacancies that fit the profile (**Relevant** = strong + possible); a switch shows each tier and **All** with counts, saved queries sit under the search box, and the search box accepts queries such as `title:(engineer | инженер) + (ai | ии) - crypto`. Each card says why it fits or not; the **Fit** tab explains every reason. See [profile relevance](RELEVANCE.md).
