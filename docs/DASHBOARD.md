...
The current visual default is dashboard design v2. The footer **Design** selector switches between v1 and v2 immediately and remembers the choice in `localStorage` under `career-copilot-design`. To choose via a link, open the dashboard with `?design=v1` or `?design=v2`; that choice is applied on load and persisted. An invalid value falls back to the saved valid choice, then to v2. Hash routes and record links are unchanged. If browser storage is unavailable, the selected design still applies for the current page.
...
- **Layout.** A navy masthead holds the sections (with record counts) and the language, text and refresh controls; on a phone the section row scrolls sideways. A compact heading introduces each page, followed by spaced cards on a light background. Readable labels, rounded controls and navy, blue, orange and sage accents distinguish actions and states. Cards and salary lines wrap on narrow screens; mobile form fields use larger text. Fonts are system stacks (Inter Tight, Inter and JetBrains Mono are used only if installed locally), so no font files are downloaded.
- **Design versions.** v2 is the default overlay for the unchanged v1 stylesheet. Use the footer selector or the `design` query parameter to compare versions. Switching only changes browser presentation; it does not change the journal, requests, routes or data.
