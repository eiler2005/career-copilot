...
(() => {
  ...
  function prepSummary(list) {
    ...
    if (overviewRecord) { const ids = [...(overviewRecord.payload.plan?.weeks || []).flatMap((week) => (week.exercises || []).map((item) => item.id)), ...(overviewRecord.payload.questions || []).map((item) => item.id)]; const progress = progressOf(ids, planStatuses(overviewRecord)); total += progress.total; attempted += progress.attempted; reviewed += progress.reviewed; }
    ...
    } else if (overviewRecord && (() => { const statuses = planStatuses(overviewRecord); for (const week of overviewRecord.payload.plan?.weeks || []) for (const exercise of week.exercises || []) if (topicState("preparation_overviews", overviewRecord.id, exercise.id, statuses) === "open") { next = {overview: overviewRecord, week, exercise}; return true; } return false; })()) {
      nextBox.append(el("p", "prep-next-title", `${t("weekShort")} ${next.week.week}: ${next.exercise.text}`), el("div", "stat-note", `${t("tests")}: ${next.exercise.tests}`), practiceButton({track: next.exercise.track === "both" ? undefined : next.exercise.track, plan_id: next.overview.id, plan_kind: "preparation_overviews", topic_id: next.exercise.id, question: next.exercise.text, type: next.exercise.type, tests: next.exercise.tests}, t("startPractice"), "primary-button"));
  ...
  const OVERVIEW_TABS = ["summary", "product", "technical-leadership", "plan", "questions", "stories", "cv"];
  ...
  function overviewView(record) {
    ...
    const ids = [...(p.plan?.weeks || []).flatMap((week) => (week.exercises || []).map((item) => item.id)), ...(p.questions || []).map((item) => item.id)];
    const header = el("header", "plan-header"), heading = el("div", "plan-heading");
    heading.append(el("p", "eyebrow", `${t("overviewEyebrow")} · ${p.as_of}`), el("h2", "plan-title", p.title), el("p", "plan-subtitle", `${t("bothTracks")} · ${p.plan?.hours_per_week} ${t("hoursShort")}/${t("weeksShort")} × ${(p.plan?.weeks || []).length} ${t("weeksShort")} · ${t("basisVacancies")}: ${(p.basis?.vacancy_ids || []).length}`));
    ...
    const draw = () => {
      tabs.replaceChildren(...OVERVIEW_TABS.map((name, index) => { const tab = button(t(`overviewTab_${name}`), `tab${name === overviewTab ? " active" : ""}`, () => { overviewTab = name; draw(); }); tab.setAttribute("role", "tab"); tab.setAttribute("aria-selected", String(name === overviewTab)); tab.tabIndex = name === overviewTab ? 0 : -1; tab.addEventListener("keydown", (event) => { const target = event.key === "ArrowRight" ? index + 1 : event.key === "ArrowLeft" ? index - 1 : null; if (target === null) return; event.preventDefault(); overviewTab = OVERVIEW_TABS[(target + OVERVIEW_TABS.length) % OVERVIEW_TABS.length]; draw(); tabs.querySelector(".tab.active")?.focus(); }); return tab; }));
